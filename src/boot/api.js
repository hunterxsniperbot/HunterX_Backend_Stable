// src/boot/api.js — API mínima, estable y con PnL/Pricing coherente para DEMO/REAL (ESM)
...
function finalizeWallet(w) {
  try { if (w && w.positions) fixupPositions(w); } catch { }
  return w;
}
...
async function buildWallet(mode = 'demo') {
  ...
  // Caja DEMO: si falta, inferimos 10k - invertido
  st.demo = st.demo || {};
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/sell') {
      ...
      if (pos.investedUsd < RESIDUAL_USD) {
        pos.isOpen = false; pos.status = 'closed'; pos.closedAt = Date.now();
        residualClosed = true;
      }
